...
- Multi-page static site (43 HTML pages, no runtime build — Vercel serves them directly)
- Shared `styles.css` (all styling) and `main.js` (all behaviour) linked by every page
- Header + footer markup is duplicated into each page (no runtime templating). The
  canonical header (with dropdown nav), mobile menu and footer live in
  `build-pages.js` — a one-off DEV generator. Run `node build-pages.js` to (1)
  regenerate every sub-page and (2) re-stamp the shared header/mobile-nav/footer
  into the 8 hand-authored pages (index, wedding, services, fleet, about,
  locations, contact, gallery). Edit the templates/data in `build-pages.js` and
  re-run rather than hand-editing nav/footer across 43 files.
- Dropdown nav mirrors the original site: Wedding, Services, Fleet, About and
  Locations each open a dropdown of real sub-pages. Desktop = CSS hover panels;
  mobile = collapsible groups (progressive enhancement — subs show without JS,
  collapse once `main.js` marks each group `js-ready`).
...
## Pages — full menu mirrors the original showlimousines.com.au (dropdowns included)
## Home · Wedding Limousines · Services · Fleet · About Us · Locations · Contact

Top-level (hand-authored, rich bodies):
...
8. `gallery.html` — masonry of real photos (`gallery-01..11.jpg`); sits under About Us

Dropdown sub-pages (generated by `build-pages.js`):
- Wedding → `wedding-sydney.html`, `wedding-wollongong.html`
- Services (8) → `service-airport-transfers`, `-cruise-transfers`, `-birthday-limos`,
  `-concert-transfers`, `-school-formals`, `-corporate-transfers`, `-party-limos`,
  `-hens-party` (`.html`)
- Fleet (11) → one page per vehicle, e.g. `fleet-rolls-royce-phantom.html`,
  `fleet-chrysler-white/-black/-black-edition/-gullwing.html`,
  `fleet-hummer-14/-16.html`, `fleet-mercedes-s-class/-valente/-sprinter.html`,
  `fleet-vw-crafter.html`
- About → `gallery.html`, `blog.html`, `reviews.html`, `terms.html`
- Locations (11) → `location-bankstown.html` … `location-wollongong.html`

Each sub-page: `.page-hero` banner → feature (copy + photo + bullets) → a relevant
secondary section (fleet strip / map / services list / testimonials / FAQ) →
`home-cta` band → shared footer. Service & location pages cross-link to vehicle
pages for SEO. Content uses only confirmed facts (5.0 / 160+ reviews, $399 start,
24/7, 365 days, Sydney & Wollongong) — no fabricated stats.
...
- Built out the FULL site to mirror the original showlimousines.com.au — added
  dropdown navigation (desktop hover panels + mobile collapsible groups) and 35
  sub-pages: 2 wedding, 8 service, 11 vehicle, 11 location, plus Blog, Reviews and
  Terms & Conditions. Introduced `build-pages.js` (dev generator) as the single
  source of truth for the shared header/mobile-nav/footer + sub-page content.
  Footer columns now link to the real sub-pages.
